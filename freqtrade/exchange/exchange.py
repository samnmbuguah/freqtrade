...
class Exchange:
    ...
    def validate_required_startup_candles(self, startup_candles: int, timeframe: str) -> int:
        ...
        Requires a grace-period of 20 candles - so a startup-period up to 49980 is allowed by default.
        """
        ...
        # Allow 100 calls to the exchange per pair
        required_candle_call_count = int(
            (candle_count / candle_limit) + (0 if candle_count % candle_limit == 0 else 1)
        )
        if self._ft_has["ohlcv_has_history"]:
            if required_candle_call_count > 100:
                # Only allow 100 calls per pair to somewhat limit the impact
                raise ConfigurationError(
                    f"This strategy requires {startup_candles} candles to start, "
                    f"which is more than 100x ({candle_limit * 100 - 1} candles) "
